...
class GameSystemsTest {
    ...
    fun throttleAcceleratesPlayer() {
        ...
        assertTrue(
            game.snapshot.player.speedKmh <= game.playerCar.maxSpeedKmh
        )
    }

    @Test
    fun steeringMovesWithinTrackBounds() {
        val game = Game()

        repeat(30) {
            game.update(
                GameInput(throttle = 1f, steer = 1f),
                1f / 60f
            )
        }

        assertTrue(game.snapshot.player.lateralOffset >= 0f)
        assertTrue(
            game.snapshot.player.lateralOffset <=
                game.track.definition.halfWidthMeters - 1.2f
        )
    }
    ...
    @Test
    fun raceStartsAtLapOneAndThreeTotalLaps() {
        val game = Game()
    ...
    fun carsOverlapUsesTrackDistance() {
        ...
        assertTrue(
            Collision().carsOverlap(
                a,
                b,
                trackLengthMeters = 140f
            )
        )
    }
}
